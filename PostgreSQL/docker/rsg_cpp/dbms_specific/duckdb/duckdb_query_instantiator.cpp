...
#include "duckdb_query_instantiator.h"
...
void DuckDBQueryInstantiator::init_data_library()
{
...
void DuckDBQueryInstantiator::constr_sql_func_lib_helper(json& json_obj, vector<string>& v_all_func_str,
...
void DuckDBQueryInstantiator::constr_sql_func_lib(string func_types_str, vector<string>& v_all_func_str,
    map<DATAAFFINITYTYPE, vector<string>>& func_type_lib,
    map<string, vector<FunctionSignature>>& func_str_to_type_map)
{
...
void DuckDBQueryInstantiator::constr_key_pair_datatype_lib_helper(json& key_pair_json, vector<string>& v_all_key_str, map<string, DataAffinity>& mapped_key_pair)
{
...
void DuckDBQueryInstantiator::constr_key_pair_datatype_lib(string key_pair_str, vector<string>& v_all_key_str, map<string, DataAffinity>& mapped_key_pair)
{
...
bool DuckDBQueryInstantiator::fill_one_stmt_helper(IR* cur_stmt)
{
...
*/
vector<IR*> DuckDBQueryInstantiator::split_to_substmt(IR* cur_stmt,
    map<IR*, pair<int, IR*>>& m_save)
{
    ...
    vector<IRTYPE> v_all_split_substmt_types { IRTypeWithClause, IRTypeCreateAsStmt, IRTypeViewStmt };
...
bool DuckDBQueryInstantiator::connect_back(map<IR*, pair<int, IR*>>& m_save)
{
...
bool DuckDBQueryInstantiator::instan_preprocessing(IR* stmt_root,
...
bool DuckDBQueryInstantiator::remove_type_annotation(IR* cur_stmt_root,
...
void DuckDBQueryInstantiator::instan_database_schema_name(IR* ir_to_fix)
{
...
void DuckDBQueryInstantiator::instan_window_name(IR* ir_to_fix)
{
...
void DuckDBQueryInstantiator::instan_table_name(IR* ir_to_fix, bool& is_replace_table)
{
...
void DuckDBQueryInstantiator::instan_table_alias_name(IR* ir_to_fix, IR* cur_stmt_root,
...
void DuckDBQueryInstantiator::instan_view_name(IR* ir_to_fix)
{
...
void DuckDBQueryInstantiator::instan_partition_name(IR* ir_to_fix)
{
...
void DuckDBQueryInstantiator::instan_index_name(IR* ir_to_fix, vector<IR*>& ir_to_drop)
{
...
DATAAFFINITYTYPE DuckDBQueryInstantiator::get_nearby_data_affinity(IR* ir_to_fix)
{
...
string DuckDBQueryInstantiator::find_cloest_table_name(IR* ir_to_fix)
{
    string closest_table_name = "";
...
void DuckDBQueryInstantiator::instan_literal(IR* ir_to_fix, IR* cur_stmt_root,
    vector<IR*>& ir_to_deep_drop)
{
    ...
    if ((type == IRTypeICONST || type == IRTypeSCONST || type == IRTypeBOOLEAN || type == IRTypeICONST)
        // && (this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeExprTuple1Ambiguous)
        // || this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeExprTupleUnambiguous))
    ) {
        ...
        if (in_expr != nullptr
            // && in_expr->get_children().front()->get_ir_type() == IRTypeExprTuple1Ambiguous
        ) {
            IR* expr_tuple_ir = nullptr;
            for (auto& tmp_child : in_expr->get_children()) {
                if (tmp_child->get_ir_type() == IRTypeExprListOptComma) {
                    expr_tuple_ir = tmp_child;
                    break;
                }
            }

            if (expr_tuple_ir != nullptr) {
                ...
                DATAAFFINITYTYPE col_affi_type;
                if (this->query_instan_data->m_column2datatype.count(col_str)) {
                    col_affi_type = this->query_instan_data->m_column2datatype[col_str]->get_data_affinity();
                } else {
                    col_affi_type = AFFIANY;
                }
                ...
                for (int in_idx = 0; in_idx < num_of_in_elem; in_idx++) {
                    IR* new_expr = new IR(IRTypeSCONST, string(""), col_affi_type, nullptr);
        ...
        //             IR* cur_expr = new IR(SymbolLit, IRTypeSCONST, string(""), nullptr);
    ...
    if (ir_to_fix->get_data_type() == DataLiteral) {
        ...
        if (ir_to_fix->get_ir_type() == IRTypeTRUEP || ir_to_fix->get_ir_type() == IRTypeFALSEP) {
#ifdef DEBUG
...
void DuckDBQueryInstantiator::instan_column_name(IR* ir_to_fix, IR*& cur_stmt_root, bool& is_replace_column, vector<IR*>& ir_to_drop)
{
    ...
    // Column name inside the TypeNameList.
    else if (ir_to_fix->get_data_type() == DataColumnName
        && ir_to_fix->get_data_flag() == ContextUse
        && this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeNameList)
        // && !this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeFamilyDef) // Not inside the FAMILY.
        ...
        if (ir_to_fix->get_data_affinity_type() != AFFIUNKNOWN) {
            ...
            if (this->query_instan_data->m_datatype2column.count(ir_to_fix->get_data_affinity_type()) == 0) {
                ...
                ir_to_fix->set_ir_type(IRTypeSCONST);
...
void DuckDBQueryInstantiator::instan_column_alias_name(IR* ir_to_fix, IR* cur_stmt_root,
    vector<IR*>& ir_to_deep_drop)
{
...
void DuckDBQueryInstantiator::instan_sql_type_name(IR* ir_to_fix)
{
...
void DuckDBQueryInstantiator::instan_statistic_name(IR* ir_to_fix)
{
...
void DuckDBQueryInstantiator::instan_sequence_name(IR* ir_to_fix)
{
...
void DuckDBQueryInstantiator::instan_constraint_name(IR* ir_to_fix)
{
...
void DuckDBQueryInstantiator::instan_family_name(IR* ir_to_fix)
{
...
IR* DuckDBQueryInstantiator::constr_rand_func_with_affinity(DATAAFFINITYTYPE in_affi)
{
...
void DuckDBQueryInstantiator::instan_func_application(IR* ir_to_fix, IR* cur_stmt_root_ir, vector<IR*>& ir_to_deep_drop,
...
void DuckDBQueryInstantiator::map_create_view(IR* ir_to_fix, IR* cur_stmt_root,
...
void DuckDBQueryInstantiator::map_create_view_column(IR* ir_to_fix,
    vector<IR*>& ir_to_deep_drop)
{
...
void DuckDBQueryInstantiator::instan_storage_param(IR* ir_to_fix, vector<IR*>& ir_to_deep_drop)
{
...
bool DuckDBQueryInstantiator::instan_dependency(IR* cur_stmt_root,
    const vector<vector<IR*>> all_substmt_ir_of_interest)
{
...
#define CHECKINIT(X)                \
    ...
    */
    for (const vector<IR*>& ir_to_fix_vec : all_substmt_ir_of_interest) {
        ...
        for (IR* ir_to_fix : ir_to_fix_vec) {
            if (ir_to_fix->get_data_type() == DataTableName && (cur_stmt_root->get_ir_type() == IRTypeCreateStmt || cur_stmt_root->get_ir_type() == IRTypeCreateAsStmt || cur_stmt_root->get_ir_type() == IRTypeViewStmt || cur_stmt_root->get_ir_type() == IRTypeIndexStmt) &&
...
#ifdef DEBUG
    cerr << "\nBEGIN duckdb_query_instantiator debugging. \n\n\n";
#endif
...
#ifdef DEBUG
    cerr << "\nEnd duckdb_query_instantiator debugging. \n\n\n";
#endif
