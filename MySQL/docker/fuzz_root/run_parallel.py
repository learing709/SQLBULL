...
def handle_background_pid_file_helper(pid_file_str: str) -> (int, bool):
    ...
    if "mysqld" not in pid_file_str or "Running" not in pid_file_str:
        return -1, True
...
for cur_inst_id in range(starting_core_id, starting_core_id + parallel_num, 1):
    ...
    mysqld_output_file = os.path.join(cur_output_dir_str, "mysqld_output.txt")
    ...
    all_mysql_server_tmux_window.append([cur_window, mysql_command, cur_mysql_data_dir_str, mysqld_output_file])
...
while True:
    ...
    for cur_window, mysql_command, cur_mysql_data_dir_str, mysqld_output_file in all_mysql_server_tmux_window:
        is_crash = False

        is_crash = handle_background_pid_file(cur_window)
        
        if is_crash:
            '''recover the backup mysql data. '''
            if os.path.isdir(cur_mysql_data_dir_str):
                shutil.rmtree(cur_mysql_data_dir_str)
            shutil.copytree(mysql_src_data_dir, cur_mysql_data_dir_str)
            ...
            with open(mysqld_output_file, "r") as mysqld_output_file_fd, open("./mysqld_output_to_fuzzer.txt", "w") as cur_output_file_fd:
                mysqld_output_file_str = mysqld_output_file_fd.read()
                cur_output_file_fd.write(mysqld_output_file_str)
            
            os.remove(mysqld_output_file)
