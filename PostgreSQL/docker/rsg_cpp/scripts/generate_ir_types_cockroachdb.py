...
def is_token_str_or_literal(cur_token: str):
    if cur_token in {"IDENT", "SCONST", "BCONST", "BITCONST", "ICONST", "FCONST"}:
        return True
    else:
        return False
...
def gen_cov_logging_func_call(cur_keyword, token_seq: str, rule_idx: int) -> str:
    ...
    for cur_token in token_seq:
        ...
        if cur_keyword_type not in all_saved_ir_types_mapping and len(cur_keyword_type) != 0:
            all_saved_ir_types_mapping[cur_keyword_type] = cur_keyword
        if cur_token_type not in all_saved_ir_types_mapping and len(cur_token_type) != 0:
            all_saved_ir_types_mapping[cur_token_type] = cur_token
    ...
    if ":" in cur_line and "':'" not in cur_line and parent_level == 0:
        ...
        if is_mistake == False:
            ...
            if cur_keyword_has_rules == False and len(cur_keyword) != 0 and "_keyword" not in cur_keyword and "error" not in cur_token_seq:
                res_has_cov += "{\n" + gen_cov_logging_func_call(cur_keyword=cur_keyword, token_seq=cur_token_seq, rule_idx=rule_idx) + "\n}\n"
            cur_token_seq = ""
...
for cur_key in all_saved_ir_types_mapping:
    cur_key = "IRType" + cur_key
    out_fd.write(f"V({cur_key}) \\\n")
...
for cur_key, cur_value in all_saved_ir_types_mapping.items():
    out_mapping_fd.write(f"{cur_value},IRType{cur_key}\n")
