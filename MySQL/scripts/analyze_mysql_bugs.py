...
bug_pattern_list = [
    ...
    "(Alter_inplace_info::ADD_VIRTUAL_COLUMN | Alter_inplace_info::ADD_STORED_BASE_COLUMN | Alter_inplace_info::ADD_STORED_GENERATED_COLUMN)",
    "escape_arg != nullptr",
    "btr0sea.cc:1083:block->page.id.space() == index->space",
    "cond->real_item()->is_bool_func()",
    "sl->join == nullptr || is_executed()",
    "MoveCompositeIteratorsFromTablePath",
    "is_prepared() && !is_optimized()",
    "query_block->leaf_table_count == 0 || thd->lex->is_query_tables_locked() || query_block == set_operand_block",
]
...
def copy_bug_folder_helper(bug_folder_idx: int):
    ...
    dest_container_name = f"mysql_testing_{bug_folder_idx}"
    # print(f"Copying bug data from container {dest_container_name}")
    os.system(f"sudo docker cp {dest_container_name}:/home/mysql/fuzzing/Bug_Analysis/detected_bugs ./bug_data_tmp")
...
def is_excluded_non_bug_queries(cur_str: str) -> bool:
    if "mysqld got signal" not in cur_str and "Assertion" not in cur_str:
        return True
    else:
        return False
