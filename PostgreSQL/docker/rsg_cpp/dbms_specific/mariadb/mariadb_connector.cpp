...
static bool terminate_query(unsigned long process_id)
{
    ...
    if (!mysql_real_connect(&tmp_m, "localhost", "mariadb", "", "test123", global_bind_to_port, global_socket_path.c_str(), 0)) {
...
bool MariaDBConnector::connect()
{
    ...
    dbname = "test123";
    if (mysql_real_connect(m_, NULL, "mariadb", "", dbname.c_str(), global_bind_to_port, global_socket_path.c_str(), 0) == NULL) {
...
bool MariaDBConnector::check_server_alive()
{
    MYSQL tmp_m;
    ...
    if (mysql_real_connect(&tmp_m, NULL, "mariadb", "", "test_init", global_bind_to_port, global_socket_path.c_str(), CLIENT_MULTI_STATEMENTS) == NULL) {
...
void MariaDBConnector::restart_dbms(char** argv)
{
    MYSQL tmp_m;
    ...
    if (mysql_real_connect(&tmp_m, NULL, "mariadb", "", "test_init", global_bind_to_port, global_socket_path.c_str(), 0) == NULL) {
    ...
    bool is_error = false;
    vector<string> v_cmd = { "SET GLOBAL TRANSACTION READ WRITE", "SET SESSION TRANSACTION READ WRITE", "RESET PERSIST", "ALTER USER 'mariadb'@'localhost' WITH MAX_USER_CONNECTIONS 0;", "DROP DATABASE IF EXISTS test123", "CREATE DATABASE IF NOT EXISTS test123", "USE test123", "SELECT 'Successful'" };
...
unsigned char MariaDBConnector::run_target(char** argv, string cmd_str,
    int is_reset_server)
{
...
bool MariaDBConnector::fix_database()
{
    MYSQL tmp_m;
...
SERVER_STATUS MariaDBConnector::clean_up_connection(MYSQL* mm)
{
    int res = -1;
    do {
...
int MariaDBConnector::retrieve_query_results_count(MYSQL& m_)
{
    MYSQL_ROW row;
    int result_count = 0;
...
string MariaDBConnector::retrieve_query_results(MYSQL* m_, string cur_cmd_str)
{
    MYSQL_ROW row;
    // string result_string = ""
    stringstream result_string_stream;
...
void MariaDBConnector::init_forkserver(char** argv)
{
    return;
}

void MariaDBConnector::write_to_testcase(string& in)
{
}

void MariaDBConnector::record_code_coverage(char** argv)
{
    /* EMPTY since trace_bits is synced with the MySQL process. No need for explicit recording. */
}

void MariaDBConnector::set_socket_path(string in)
{
    this->socket_path = in;
    global_socket_path = in;
}

void MariaDBConnector::set_bind_to_port(int in)
{
