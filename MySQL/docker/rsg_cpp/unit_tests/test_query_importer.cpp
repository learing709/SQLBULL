...
#error "Error: Cannot recognize dbms_name: either cockroachdb nor duckdb. "
    ...
    // do the unit test.
    vector<IR*> v_parsed_stmts = p_query_importer->import_grammar("test_in.sql");
    for (IR* cur_parsed_stmt : v_parsed_stmts) {
        cout << cur_parsed_stmt->to_string() << ";\n\n";
    }

    // finish the test, clean up
    for (IR* cur_parsed_stmt : v_parsed_stmts) {
        cur_parsed_stmt->deep_drop();
    }
