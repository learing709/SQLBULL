...
#include "sqlite_query_instantiator.h"
...
void SQLiteQueryInstantiator::init_data_library()
{
...
void SQLiteQueryInstantiator::constr_sql_func_lib_helper(json& json_obj, vector<string>& v_all_func_str,
...
void SQLiteQueryInstantiator::constr_sql_func_lib(string func_types_str, vector<string>& v_all_func_str,
    map<DATAAFFINITYTYPE, vector<string>>& func_type_lib,
    map<string, vector<FunctionSignature>>& func_str_to_type_map)
{
...
void SQLiteQueryInstantiator::constr_key_pair_datatype_lib_helper(json& key_pair_json, vector<string>& v_all_key_str, map<string, DataAffinity>& mapped_key_pair)
{
...
void SQLiteQueryInstantiator::constr_key_pair_datatype_lib(string key_pair_str, vector<string>& v_all_key_str, map<string, DataAffinity>& mapped_key_pair)
{
...
bool SQLiteQueryInstantiator::fill_one_stmt_helper(IR* cur_stmt)
{
...
*/
vector<IR*> SQLiteQueryInstantiator::split_to_substmt(IR* cur_stmt,
    map<IR*, pair<int, IR*>>& m_save)
{
    ...
    vector<IRTYPE> v_all_split_substmt_types { IRTypeSelectStmt, IRTypeSelect };
...
bool SQLiteQueryInstantiator::connect_back(map<IR*, pair<int, IR*>>& m_save)
{
...
bool SQLiteQueryInstantiator::instan_preprocessing(IR* stmt_root,
...
bool SQLiteQueryInstantiator::remove_type_annotation(IR* cur_stmt_root,
...
void SQLiteQueryInstantiator::instan_database_schema_name(IR* ir_to_fix)
{
...
void SQLiteQueryInstantiator::instan_window_name(IR* ir_to_fix)
{
...
void SQLiteQueryInstantiator::instan_table_name(IR* ir_to_fix, bool& is_replace_table)
{
    ...
    if ((ir_to_fix->get_data_type() == DataTableName) && (ir_to_fix->get_data_flag() == ContextDefine || ir_to_fix->get_data_flag() == ContextReplaceDefine)) {
        ...
        // if (this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeFromTables)) {
        if (this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeFrom)) {
            is_aliased = true;
...
void SQLiteQueryInstantiator::instan_table_alias_name(IR* ir_to_fix, IR* cur_stmt_root,
...
void SQLiteQueryInstantiator::instan_view_name(IR* ir_to_fix)
{
...
void SQLiteQueryInstantiator::instan_partition_name(IR* ir_to_fix)
{
...
void SQLiteQueryInstantiator::instan_index_name(IR* ir_to_fix, vector<IR*>& ir_to_drop)
{
...
DATAAFFINITYTYPE SQLiteQueryInstantiator::get_nearby_data_affinity(IR* ir_to_fix)
{
    ...
    vector<IRTYPE> v_capped_ir_types { IRTypeSelect, IRTypeSelectStmt, IRTypeCmd };
...
string SQLiteQueryInstantiator::find_cloest_table_name(IR* ir_to_fix)
{
    string closest_table_name = "";
...
void SQLiteQueryInstantiator::instan_literal(IR* ir_to_fix, IR* cur_stmt_root,
    vector<IR*>& ir_to_deep_drop)
{
    ...
    if (ir_to_fix->get_data_type() == DataLiteral || type == IRTypeINTEGER || type == IRTypeFLOAT || type == IRTypeBOOLEAN || type == IRTypeSTRING || type == IRTypeCTIME_KW || type == IRTypeBLOB) {
...
void SQLiteQueryInstantiator::instan_column_name(IR* ir_to_fix, IR*& cur_stmt_root, bool& is_replace_column, vector<IR*>& ir_to_drop)
{
    ...
    /* ContextUndefine scenario of the DataColumnName */
        ...
        if (cur_mapped_column_name_vec.size() > 0) {
            ...
            if (this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeSortlist) || !this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeSelectStmt)) {
...
void SQLiteQueryInstantiator::instan_column_alias_name(IR* ir_to_fix, IR* cur_stmt_root,
    vector<IR*>& ir_to_deep_drop)
{
    ...
    if (ir_to_fix->get_data_type() == DataColumnAliasName) {
        ...
        if (this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeAs)) {
...
void SQLiteQueryInstantiator::instan_sql_type_name(IR* ir_to_fix)
{
...
void SQLiteQueryInstantiator::instan_statistic_name(IR* ir_to_fix)
{
...
void SQLiteQueryInstantiator::instan_sequence_name(IR* ir_to_fix)
{
...
void SQLiteQueryInstantiator::instan_constraint_name(IR* ir_to_fix)
{
...
void SQLiteQueryInstantiator::instan_family_name(IR* ir_to_fix)
{
...
IR* SQLiteQueryInstantiator::constr_rand_func_with_affinity(DATAAFFINITYTYPE in_affi)
{
    ...
    for (DataAffinity& cur_arg_affi : v_func_affi) {
        ...
        if (is_col_used_ok && get_pct_hit(66)) {
            ...
            auto* new_dump_expr = new IR(SymbolNonTerm, IRTypeExpr, string(), p_rsg->get_token_from_ir_type(IRTypeExpr), &dump_simple_expr_node, p_rsg->get_prod_from_ir_type(IRTypeExpr));
    ...
    IR* ret_ir = new IR(SymbolNonTerm, IRTypeExpr, string(), p_rsg->get_token_from_ir_type(IRTypeExpr));
...
void SQLiteQueryInstantiator::instan_func_application(IR* ir_to_fix, IR* cur_stmt_root_ir, vector<IR*>& ir_to_deep_drop,
...
void SQLiteQueryInstantiator::map_create_view(IR* ir_to_fix, IR* cur_stmt_root,
    const vector<vector<IR*>> all_substmt_ir_of_interest)
{
    ...
    if (cur_stmt_root->get_children_ref().size() > 3 && cur_stmt_root->get_children_ref()[0]->get_ir_type() == IRTypeCreatekw && cur_stmt_root->get_children_ref()[2]->get_ir_type() == IRTypeVIEW) {
...
void SQLiteQueryInstantiator::map_create_view_column(IR* ir_to_fix,
    vector<IR*>& ir_to_deep_drop)
{

    IR* type_name_list = this->ir_wrapper.get_ancestor_node_matching_type(ir_to_fix, IRTypeEidlistOpt);
...
void SQLiteQueryInstantiator::instan_storage_param(IR* ir_to_fix, vector<IR*>& ir_to_deep_drop)
{
...
void SQLiteQueryInstantiator::instan_trigger_name(IR* ir_to_fix, vector<IR*>& ir_to_deep_drop) {
...
bool SQLiteQueryInstantiator::instan_dependency(IR* cur_stmt_root,
    const vector<vector<IR*>> all_substmt_ir_of_interest)
{
    ...
    if (X->get_is_instantiated()) { \
        ...
        for (IR* ir_to_fix : ir_to_fix_vec) {
            ...
            if (ir_to_fix->get_data_type() == DataTableAliasName) {
                ...
                if (!this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeWith)) {
                    continue;
                }
        ...
        for (IR* ir_to_fix : ir_to_fix_vec) {
            ...
            if (ir_to_fix->get_data_type() == DataColumnName && ir_to_fix->get_data_flag() == ContextUse && this->ir_wrapper.is_ir_in<IRTYPE>(ir_to_fix, IRTypeEidlist)) {
                this->instan_column_name(ir_to_fix, cur_stmt_root, is_replace_column,
                    ir_to_deep_drop);
            }
        }
...
#ifdef DEBUG
    cerr << "\nBEGIN sqlite_query_instantiator debugging. \n\n\n";
#endif
...
#ifdef DEBUG
    cerr << "\nEnd sqlite_query_instantiator debugging. \n\n\n";
#endif

    return !is_conflict;
}

void SQLiteQueryInstantiator::custom_setup_init_query_instan_data() {

    query_instan_data->v_table_names.emplace_back("v03");
    query_instan_data->m_table2columns["v03"].emplace_back("tree_id");
    query_instan_data->m_table2columns["v03"].emplace_back("minX");
    query_instan_data->m_table2columns["v03"].emplace_back("maxX");
    query_instan_data->m_table2columns["v03"].emplace_back("minY");
    query_instan_data->m_table2columns["v03"].emplace_back("maxY");
    query_instan_data->m_column2datatype["tree_id"] = new DataAffinity(DATAAFFINITYTYPE::AFFIINT);
    query_instan_data->m_column2datatype["minX"] = new DataAffinity(DATAAFFINITYTYPE::AFFIFLOAT);
    query_instan_data->m_column2datatype["maxX"] = new DataAffinity(DATAAFFINITYTYPE::AFFIFLOAT);
    query_instan_data->m_column2datatype["minY"] = new DataAffinity(DATAAFFINITYTYPE::AFFIFLOAT);
    query_instan_data->m_column2datatype["maxY"] = new DataAffinity(DATAAFFINITYTYPE::AFFIFLOAT);
    query_instan_data->m_datatype2column[DATAAFFINITYTYPE::AFFIINT].emplace_back("tree_id");
    query_instan_data->m_datatype2column[DATAAFFINITYTYPE::AFFIFLOAT].emplace_back("minX");
    query_instan_data->m_datatype2column[DATAAFFINITYTYPE::AFFIFLOAT].emplace_back("maxX");
    query_instan_data->m_datatype2column[DATAAFFINITYTYPE::AFFIFLOAT].emplace_back("minY");
    query_instan_data->m_datatype2column[DATAAFFINITYTYPE::AFFIFLOAT].emplace_back("maxY");

    query_instan_data->v_table_names.emplace_back("v04");
    query_instan_data->m_table2columns["v04"].emplace_back("c05");
    query_instan_data->m_table2columns["v04"].emplace_back("c06");
    query_instan_data->m_column2datatype["c05"] = new DataAffinity(DATAAFFINITYTYPE::AFFIANY);
    query_instan_data->m_column2datatype["c06"] = new DataAffinity(DATAAFFINITYTYPE::AFFIANY);
    query_instan_data->m_datatype2column[DATAAFFINITYTYPE::AFFIINT].emplace_back("c05");
    query_instan_data->m_datatype2column[DATAAFFINITYTYPE::AFFIFLOAT].emplace_back("c05");
    query_instan_data->m_datatype2column[DATAAFFINITYTYPE::AFFISTRING].emplace_back("c05");
    query_instan_data->m_datatype2column[DATAAFFINITYTYPE::AFFIINT].emplace_back("c06");
    query_instan_data->m_datatype2column[DATAAFFINITYTYPE::AFFIFLOAT].emplace_back("c06");
    query_instan_data->m_datatype2column[DATAAFFINITYTYPE::AFFISTRING].emplace_back("c06");

    query_instan_data->v_table_names.emplace_back("v07");
    query_instan_data->m_table2columns["v07"].emplace_back("c08");
    query_instan_data->m_column2datatype["c08"] = new DataAffinity(DATAAFFINITYTYPE::AFFIINT);
    query_instan_data->m_datatype2column[DATAAFFINITYTYPE::AFFIINT].emplace_back("c08");

}

vector<QueryStmt*> SQLiteQueryInstantiator::construct_custom_pre_insert_stmt() {

    // insert random pragma statements. 

    vector<QueryStmt*> res_stmt_vec;

    for (int i = 0; i < 3; i++) {
        string res = "";

        string key = "";
        int lib_size = v_cmds_.size();
        key = v_cmds_[get_rand_int(lib_size)];
        res += ("PRAGMA " + key);

        int value_size = m_cmd_value_lib_[key].size();
        string value = m_cmd_value_lib_[key][get_rand_int(value_size)];
        if (!value.compare("_int_")) {
        string tmp_value_lib = value_library_[get_rand_int(value_library_.size())];
        ...
        if (get_rand_int(2) == 0)
            ...
            value = "=true";
        } else if (!value.compare("_neg_int_")) { 
            string tmp_value_lib = value_library_[get_rand_int(value_library_.size())];
            if (tmp_value_lib.empty()) {
                tmp_value_lib = "0.0";
            }
        ...
        QueryStmt* res_stmt = new QueryStmt(res);

        res_stmt_vec.push_back(res_stmt);
    }
