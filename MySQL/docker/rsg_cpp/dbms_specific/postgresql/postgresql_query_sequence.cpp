...
#include "postgresql_query_sequence.h"
...
QueryStmt* generate_stmt_with_idx(int gen_idx, RSG* p_rsg)
{
    if (gen_idx < 4) {
        ...
        if (get_pct_hit(80)) {
            return p_rsg->generate_stmt_with_stmt_type(IRTypeAlterTableStmt);
        } else {
            return p_rsg->generate_stmt_with_stmt_type(IRTypeIndexStmt);
        }
    } else if (gen_idx < 18) {
        return p_rsg->generate_stmt_with_stmt_type(IRTypeIndexStmt);
    } else if (gen_idx < 22) {
...
QueryStmt* PostgreSQLQuerySequenceGenerator::generate_next_query_helper(RSG* p_rsg, FuzzingSequenceQueue* p_queue)
{
    if (this->gen_mode == GenUnknown) {
        // If already saved 100 query sequences, then mutate only.
        if (get_pct_hit(PostgreSQLFuzzerConfigurations::pct_mutate_ratio) || p_queue->get_saved_query_sequence_size() >= 100) {
    ...
    if (this->gen_mode == GenAllFromNew) {
        ...
        if (this->gen_idx >= this->p_saved_query_sequence->v_good_query_stmts.size()) {
            ...
            this->gen_idx++;
        } else if (get_pct_hit(PostgreSQLFuzzerConfigurations::pct_insert_new_in_mut) && this->p_saved_query_sequence->v_good_query_stmts.size() < 40) {
...
PostgreSQLQuerySequenceGenerator::PostgreSQLQuerySequenceGenerator()
{
