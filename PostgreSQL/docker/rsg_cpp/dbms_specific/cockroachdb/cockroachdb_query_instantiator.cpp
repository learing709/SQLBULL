...
#include "cockroachdb_query_instantiator.h"
...
void CockroachDBQueryInstantiator::init_data_library()
{
    ...
    string set_session_path = FuzzerConfigurations::SET_SESSION_PATH;
    ...
    string storage_parameter_path = FuzzerConfigurations::STORAGE_PARAM_PATH;
...
void CockroachDBQueryInstantiator::constr_sql_func_lib_helper(json& json_obj, vector<string>& v_all_func_str,
    ...
    map<string, vector<FunctionSignature>>& func_str_to_type_map)
{
    ...
    for (json::iterator it = json_obj.begin(); it != json_obj.end(); it++) {
        ...
        for (json::iterator it_params = params_node.begin(); it_params != params_node.end(); it_params++) {
            ...
            ret_type.set_data_affinity(get_dataaffi_by_string(it_params->at("ret_type")));
            single_signature.return_data_affi = ret_type;
            ...
            for (int i = 0; i < it_params->size(); i++) {
                ...
                cur_arg_type.set_data_affinity(get_dataaffi_by_string(it_params->at(arg_key)));
...
void CockroachDBQueryInstantiator::constr_sql_func_lib(string func_types_str, vector<string>& v_all_func_str,
    map<DATAAFFINITYTYPE, vector<string>>& func_type_lib,
    map<string, vector<FunctionSignature>>& func_str_to_type_map)
{
...
void CockroachDBQueryInstantiator::constr_key_pair_datatype_lib_helper(json& key_pair_json, vector<string>& v_all_key_str, map<string, DataAffinity>& mapped_key_pair)
{
...
void CockroachDBQueryInstantiator::constr_key_pair_datatype_lib(string key_pair_str, vector<string>& v_all_key_str, map<string, DataAffinity>& mapped_key_pair)
{
...
bool CockroachDBQueryInstantiator::fill_one_stmt_helper(IR* cur_stmt)
{
...
*/
vector<IR*> CockroachDBQueryInstantiator::split_to_substmt(IR* cur_stmt,
    map<IR*, pair<int, IR*>>& m_save)
{
    ...
    vector<IRTYPE> v_all_split_substmt_types { IRTypeWithClause, IRTypeCreateViewStmt, IRTypeCreateTableAsStmt };
...
bool CockroachDBQueryInstantiator::connect_back(map<IR*, pair<int, IR*>>& m_save)
{
...
bool CockroachDBQueryInstantiator::instan_preprocessing(IR* stmt_root,
...
bool CockroachDBQueryInstantiator::remove_type_annotation(IR* cur_stmt_root,
    vector<IR*>& ir_to_deep_drop)
{
    vector<IR*> v_type_annotate_ir = this->ir_wrapper.get_ir_node_in_stmt_with_type(cur_stmt_root, IRTypeTYPEANNOTATE,
        false, true);
    for (IR*& cur_annotate : v_type_annotate_ir) {
        if (cur_annotate->get_parent_node()->get_ir_type() == IRTypeAExpr
            || cur_annotate->get_parent_node()->get_ir_type() == IRTypeBExpr) {
            IR* parent_node = cur_annotate->get_parent_node();
            ir_to_deep_drop.push_back(parent_node->get_children().back());
            ir_to_deep_drop.push_back(cur_annotate);
            parent_node->detach_one_child(parent_node->get_children().back());
            parent_node->detach_one_child(cur_annotate);
        }
    }

    vector<IR*> v_type_cast_ir = this->ir_wrapper.get_ir_node_in_stmt_with_type(cur_stmt_root, IRTypeCAST,
        false, true);
    for (IR*& cur_cast : v_type_cast_ir) {
        if (cur_cast->get_parent_node()->get_ir_type() == IRTypeAExpr
            || cur_cast->get_parent_node()->get_ir_type() == IRTypeBExpr) {
            IR* parent_node = cur_cast->get_parent_node();
            ir_to_deep_drop.push_back(parent_node->get_children().back());
            ir_to_deep_drop.push_back(cur_cast);
            parent_node->detach_one_child(parent_node->get_children().back());
            parent_node->detach_one_child(cur_cast);
        }
    }
    return true;
}

void CockroachDBQueryInstantiator::instan_database_schema_name(IR* ir_to_fix)
{
    if (ir_to_fix->get_data_type() == DataDatabaseName) {
        ir_to_fix->set_str_val("movr");
    } else if (ir_to_fix->get_data_type() == DataSchemaName) {
        ir_to_fix->set_str_val("public");
    }
}

void CockroachDBQueryInstantiator::instan_window_name(IR* ir_to_fix)
{
...
void CockroachDBQueryInstantiator::instan_table_name(IR* ir_to_fix, bool& is_replace_table)
{
    if (this->ir_wrapper.is_ir_in<IRTYPE>(ir_to_fix, IRTypeSetClause) || this->ir_wrapper.is_ir_in<IRTYPE>(ir_to_fix, IRTypeStorageParameter) || this->ir_wrapper.is_ir_in<IRTYPE>(ir_to_fix, IRTypeIndexFlagsParam)) {
        return;
    }
...
void CockroachDBQueryInstantiator::instan_table_alias_name(IR* ir_to_fix, IR* cur_stmt_root,
...
void CockroachDBQueryInstantiator::instan_view_name(IR* ir_to_fix)
{

    if (this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeSetClause) || this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeStorageParameter) || this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeIndexFlagsParam)) {
        return;
    }
...
void CockroachDBQueryInstantiator::instan_partition_name(IR* ir_to_fix)
{
...
void CockroachDBQueryInstantiator::instan_index_name(IR* ir_to_fix, vector<IR*>& ir_to_drop)
{
...
DATAAFFINITYTYPE CockroachDBQueryInstantiator::get_nearby_data_affinity(IR* ir_to_fix)
{
    ...
    vector<IRTYPE> v_capped_ir_types { IRTypeSelectClause, IRTypeSelectStmt };
    ...
    if (near_literal_node != NULL && near_literal_node->get_data_affinity_type() != AFFIUNKNOWN && near_literal_node->get_data_affinity_type() != AFFIANY && near_literal_node->get_is_instantiated()) {
        ...
        if (nearby_column_ir != NULL) {
            ...
            if (this->query_instan_data->m_column2datatype.count(nearby_column_str) || this->query_instan_data->m_alias2column_single.count(nearby_column_str)) {
                ...
                DataAffinity* cur_affi = this->query_instan_data->m_column2datatype[actual_column_str];
                ...
                cerr << "Dependency: INFO: From Literal handling, getting "
                     ...
                     << get_string_by_data_affi(cur_affi->get_data_affinity())
                     << "\n\n\n";
#endif
            } else {
...
string CockroachDBQueryInstantiator::find_cloest_table_name(IR* ir_to_fix)
{
    string closest_table_name = "";
...
void CockroachDBQueryInstantiator::instan_literal(IR* ir_to_fix, IR* cur_stmt_root,
    vector<IR*>& ir_to_deep_drop)
{
    ...
    if ((type == IRTypeICONST || type == IRTypeSCONST || type == IRTypeBOOLEAN || type == IRTypeICONST) && (this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeExprTuple1Ambiguous) || this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeExprTupleUnambiguous))) {
        ...
        // FIXME:: Handle the IRTypeValuesClause.
        //        // Handle the ValuesClause.
        //        // Get the TypeExprsNode first.
        //        IR* type_exprs_node = this->ir_wrapper.get_ancestor_node_matching_type(ir_to_fix, IRTypeExprList);
        //
        // #ifdef DEBUG
        ...
        // #endif
        //
        //        for (IR*& cur_expr : type_exprs_node->get_children_ref()) {
        //            ir_to_deep_drop.push_back(cur_expr);
        //        }
        //        type_exprs_node->detach_children();

        /* Reconstruct the new type expressions clause that matched the referenced
         * table.
         */

        IR* in_expr = this->ir_wrapper.get_ancestor_node_matching_type(ir_to_fix, IRTypeInExpr);
        if (in_expr != nullptr && in_expr->get_children().front()->get_ir_type() == IRTypeExprTuple1Ambiguous) {
            IR* expr_tuple_ir = in_expr->get_children().front();
            /* Fix for the IN clause. */
            vector<DATATYPE> search_type { DataColumnName, DataColumnAliasName };
            vector<IRTYPE> cap_type { IRTypeSelectStmt };
            IR* closet_column_node = this->ir_wrapper.find_closest_nearby_IR_with_type(
                ir_to_fix, search_type, cap_type);
...
#endif
                IR* new_dummy_node = new IR(IRTypeICONST, 0, AFFIINT, nullptr);
                new_dummy_node->mutate_literal_with_type(AFFIINT);
                new_dummy_node->set_is_instantiated(true);
                vector<IR*> new_dummy_vec { new_dummy_node };
                for (IR*& ori_expr : expr_tuple_ir->get_children_ref()) {
                    ir_to_deep_drop.push_back(ori_expr);
                }
                expr_tuple_ir->detach_children();
                expr_tuple_ir->set_children_nodes(new_dummy_vec);
                return;
            }

            string col_str = closet_column_node->get_str_val();
            DataAffinity* col_affi = this->query_instan_data->m_column2datatype[col_str];

            // Avoid 0.
            int num_of_in_elem = get_rand_int(5) + 1;

            string ret_str = "";
            vector<IR*> new_expr_children;
            for (int in_idx = 0; in_idx < num_of_in_elem; in_idx++) {
                IR* new_expr = new IR(IRTypeSCONST, string(""), col_affi->get_data_affinity(), nullptr);
                new_expr->mutate_literal();
                new_expr_children.push_back(new_expr);
                new_expr->set_is_instantiated(true);
                new_expr->set_data_flag(ContextNoModi);
                if (in_idx != num_of_in_elem - 1) {
                    IR* node_sep = new IR(SymbolTerm, IRTypeUnknownType, string(","), nullptr);
                    new_expr_children.push_back(node_sep);
                }
            }
            for (IR*& ori_expr : expr_tuple_ir->get_children_ref()) {
                ir_to_deep_drop.push_back(ori_expr);
            ...
            expr_tuple_ir->detach_children();
            expr_tuple_ir->set_children_nodes(new_expr_children);

#ifdef DEBUG
            cerr << "\n\n\nDependency: getting new IN clause expression: "
                 << expr_tuple_ir->to_string() << ". \n\n\n";
#endif

            return;
        } // IN and VALUES clause
        ...
        IR* name_list_ir = this->ir_wrapper.find_closest_nearby_IR_with_type(cur_stmt_root,
            IRTypeNameList);

        vector<DataAffinity> referencing_affinity;
        if (name_list_ir == nullptr) {
...
#endif
                string cur_table_name = this->query_instan_data->v_table_names_single.front();
                vector<string> column_list;
                bool is_alias = false;
                if (this->query_instan_data->m_alias_table2column_single.count(cur_table_name) > 0) {
                    is_alias = true;
                    column_list = this->query_instan_data->m_alias_table2column_single[cur_table_name];
                } else {
                    is_alias = false;
                    column_list = this->query_instan_data->m_table2columns[cur_table_name];
                }
                for (const string& cur_column_str : column_list) {
                    string actual_column_str = cur_column_str;
                    if (is_alias && this->query_instan_data->m_alias2column_single.count(actual_column_str) > 0) {
#ifdef DEBUG
                        cerr << "\n\n\nDependency: INFO: In literal fixing, mapping the "
                                "column alias: "
                             ...
                             << "\n\n\n";
#endif
                        actual_column_str = this->query_instan_data->m_alias2column_single[cur_column_str];
                    }
                    if (this->query_instan_data->m_column2datatype.count(actual_column_str) > 0) {
                        DataAffinity* cur_affi = this->query_instan_data->m_column2datatype[actual_column_str];
                        referencing_affinity.push_back(*cur_affi);
#ifdef DEBUG
            ...
            cerr << "\n\n\nDependency: Find the column name list from the stmt:"
                 << name_list_ir->to_string() << ". \n\n\n";
#endif

            IR* type_list_node = name_list_ir;
            vector<string>& v_column_str = this->query_instan_data->v_column_names_single;
            if (v_column_str.size() != 0) {
                for (string& cur_column_str : v_column_str) {
                    string actual_column_str = cur_column_str;
                    if (this->query_instan_data->m_alias2column_single.count(cur_column_str)) {
                        actual_column_str = this->query_instan_data->m_alias2column_single[cur_column_str];
#ifdef DEBUG
                        cerr << "\n\n\nDependency: INFO: In literal fixing, mapping the "
                                "column alias: "
                ...
                vector<IR*> v_column_node = this->ir_wrapper.get_ir_node_in_stmt_with_type(
                    type_list_node, DataColumnName, false);
                for (IR* cur_column_node : v_column_node) {
                    string cur_column_str = cur_column_node->get_str_val();
                    if (this->query_instan_data->m_column2datatype.count(cur_column_str) || this->query_instan_data->m_alias2column_single.count(cur_column_str)) {
                        string actual_column_str = cur_column_str;
                        if (this->query_instan_data->m_alias2column_single.count(cur_column_str)) {
                            actual_column_str = this->query_instan_data->m_alias2column_single[cur_column_str];
#ifdef DEBUG
        ...
        vector<IR*> children_vec;
        for (int idx = 0; idx < referencing_affinity.size(); idx++) {
            IR* cur_expr = new IR(SymbolLit, IRTypeSTRING, string(""), nullptr);
            cur_expr->set_data_affinity(referencing_affinity[idx]);
            cur_expr->mutate_literal();
            cur_expr->set_data_type(DataLiteral);
            cur_expr->set_data_flag(ContextNoModi);
            cur_expr->set_is_instantiated(true);
            children_vec.push_back(cur_expr);

            if (idx != referencing_affinity.size() - 1) {
                IR* sep_node = new IR(SymbolTerm, IRTypeUnknownType, string(","), nullptr);
                children_vec.push_back(sep_node);
            }
        }

        for (IR*& cur_expr : name_list_ir->get_children_ref()) {
            ir_to_deep_drop.push_back(cur_expr);
        }
        name_list_ir->detach_children();
        name_list_ir->set_children_nodes(children_vec);
    ...
    if (ir_to_fix->get_data_type() == DataLiteral) {
        ...
        if (ir_to_fix->get_ir_type() == IRTypeTRUE || ir_to_fix->get_ir_type() == IRTypeFALSE) {
#ifdef DEBUG
...
void CockroachDBQueryInstantiator::instan_column_name(IR* ir_to_fix, IR*& cur_stmt_root, bool& is_replace_column, vector<IR*>& ir_to_drop)
{
    ...
    else if (ir_to_fix->get_data_type() == DataColumnName && ir_to_fix->get_data_flag() == ContextUndefine) {
    ...
    // Column name inside the TypeNameList.
    else if (ir_to_fix->get_data_type() == DataColumnName && ir_to_fix->get_data_flag() == ContextUse && this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeNameList) && !this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeFamilyDef) && // Not inside the FAMILY.
        !this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeUpdateStmt)) {
        ...
        if (ir_to_fix->get_data_affinity_type() != AFFIUNKNOWN) {
            ...
            if (this->query_instan_data->m_datatype2column.count(ir_to_fix->get_data_affinity_type()) == 0) {
                ...
                // as an literal.
                ir_to_fix->set_symbol_type(SymbolLit);
                ir_to_fix->set_ir_type(IRTypeSTRING);
                ir_to_fix->set_data_type(DataUnknownType);
                ir_to_fix->mutate_literal_with_type(AFFISTRING);
                return;
            }
            string cur_chosen_col = vector_rand_ele(this->query_instan_data->m_datatype2column[ir_to_fix->get_data_affinity_type()]);
        ...
        IR* name_list = this->ir_wrapper.get_ancestor_node_matching_type(ir_to_fix, IRTypeNameList);

        string closest_table_name = this->find_cloest_table_name(name_list);
        ...
        if (this->query_instan_data->m_alias_table2column_single.count(closest_table_name) > 0) {
            v_column_names_from_table = this->query_instan_data->m_alias_table2column_single[closest_table_name];
        } else {
            v_column_names_from_table = this->query_instan_data->m_table2columns[closest_table_name];
        }
        ...
        for (int idx = 0; idx < max_values_clause_len;) {
            ...
            IR* new_column_node = new IR(IRTypeName, string(new_rand_column),
                DataColumnName, ContextNoModi, nullptr);

            v_new_column_list_node.push_back(new_column_node);
            this->query_instan_data->v_column_names_single.push_back(new_rand_column);
        ...
        for (IR*& ori_name : name_list->get_children_ref()) {
            ir_to_drop.push_back(ori_name);
        }

        name_list->detach_children();

        vector<IR*> v_new_column_list_node_with_sep;
        v_new_column_list_node_with_sep.reserve(v_new_column_list_node.size() + 1);
        for (int idx = 0; idx < v_new_column_list_node.size(); idx++) {
            v_new_column_list_node_with_sep.push_back(v_new_column_list_node[idx]);
            v_new_column_list_node[idx]->set_is_instantiated(true);
            v_new_column_list_node[idx]->set_data_flag(ContextNoModi);
            if (idx != v_new_column_list_node.size() - 1) {
                IR* sep_ir = new IR(SymbolTerm, IRTypeUnknownType, string(","), nullptr);
                v_new_column_list_node_with_sep.push_back(sep_ir);
            }
        }

        name_list->set_children_nodes(v_new_column_list_node_with_sep);
        name_list->set_is_instantiated(true);

#ifdef DEBUG
        cerr << "   replaced to new name list: "
             << name_list->get_parent_node()->to_string() << "\n\n\n";
#endif

        return;
    }

    else if (ir_to_fix->get_data_type() == DataColumnName && ir_to_fix->get_data_flag() == ContextUse) {
#ifdef DEBUG
        ...
        if (cur_mapped_column_name_vec.size() > 0) {
            ...
            if (!this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeValuesClause)) {
...
void CockroachDBQueryInstantiator::instan_column_alias_name(IR* ir_to_fix, IR* cur_stmt_root,
    vector<IR*>& ir_to_deep_drop)
{
        ...
        if (is_alias_clause) {
            ...
            if (select_subquery != NULL && select_subquery != cur_stmt_root) {
                ...
                IR* opt_alias_clause = this->ir_wrapper.get_ancestor_node_matching_type(
                    ir_to_fix, IRTypeOptAliasClause);
...
void CockroachDBQueryInstantiator::instan_sql_type_name(IR* ir_to_fix)
{
...
void CockroachDBQueryInstantiator::instan_statistic_name(IR* ir_to_fix)
{
...
void CockroachDBQueryInstantiator::instan_sequence_name(IR* ir_to_fix)
{
...
void CockroachDBQueryInstantiator::instan_constraint_name(IR* ir_to_fix)
{
...
void CockroachDBQueryInstantiator::instan_family_name(IR* ir_to_fix)
{
...
IR* CockroachDBQueryInstantiator::constr_rand_func_with_affinity(DATAAFFINITYTYPE in_affi)
{
    ...
    IR* args_list_ir = new IR(SymbolNonTerm, IRTypeExprList, string(), p_rsg->get_token_from_ir_type(IRTypeExprList), arg_list_prod_node->get_exprs().back(), arg_list_prod_node);
    ...
    for (DataAffinity& cur_arg_affi : v_func_affi) {
        ...
        if (is_col_used_ok && get_pct_hit(66)) {
            IR* new_col_ir = new IR(IRTypeColumnName, cur_col_str, DataColumnName, ContextUse, nullptr);
            args_list_ir->get_children_ref().push_back(new_col_ir);
            new_col_ir->set_parent_node(args_list_ir);
            ...
            IR* new_literal_ir = new IR(SymbolLit, IRTypeSCONST, cur_arg_str, nullptr);
            new_literal_ir->set_data_affinity_type(cur_arg_affi.get_data_affinity());
            args_list_ir->get_children_ref().push_back(new_literal_ir);
            new_literal_ir->set_parent_node(args_list_ir);
    ...
    IR* ret_ir = new IR(SymbolNonTerm, IRTypeFuncApplication, string(), p_rsg->get_token_from_ir_type(IRTypeFuncExpr));

    IR* func_name_ir = new IR(IRTypeFuncName, cur_func_name, DataFunctionName, ContextUse, nullptr);
...
void CockroachDBQueryInstantiator::instan_func_application(IR* ir_to_fix, IR* cur_stmt_root_ir, vector<IR*>& ir_to_deep_drop,
    bool is_ignore_nested_expr)
{
    ...
    if (this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeSetClause) || this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeStorageParameter)) {
#ifdef DEBUG
...
void CockroachDBQueryInstantiator::map_create_view(IR* ir_to_fix, IR* cur_stmt_root,
    const vector<vector<IR*>> all_substmt_ir_of_interest)
{

    if (ir_to_fix->get_data_type() != DataTableName && ir_to_fix->get_data_type() != DataViewName) {
        return;
    }
    ...
    if (cur_stmt_root->get_ir_type() == IRTypeCreateViewStmt) {
    ...
    if (is_in_create_view) {
        ...
        for (IR* cur_mentioned_alias : all_mentioned_column_alias_vec) {
            if (this->ir_wrapper.is_ir_in(cur_mentioned_alias, IRTypeTargetElem)) {
...
void CockroachDBQueryInstantiator::map_create_view_column(IR* ir_to_fix,
    vector<IR*>& ir_to_deep_drop)
{

    IR* type_name_list = this->ir_wrapper.get_ancestor_node_matching_type(ir_to_fix, IRTypeOptColumnList);
    ...
    for (string cur_matched_columns : matched_columns) {
        ...
        IR* new_view_column = new IR(SymbolTerm, IRTypeColumnName, new_view_column_name, nullptr);
...
void CockroachDBQueryInstantiator::instan_storage_param(IR* ir_to_fix, vector<IR*>& ir_to_deep_drop)
{
...
bool CockroachDBQueryInstantiator::instan_dependency(IR* cur_stmt_root,
    const vector<vector<IR*>> all_substmt_ir_of_interest)
{
...
#define CHECKINIT(X)                \
    if (X->get_is_instantiated()) { \
        ...
        for (IR* ir_to_fix : ir_to_fix_vec) {
            ...
            if (ir_to_fix->get_data_type() == DataColumnName && ir_to_fix->get_data_flag() == ContextUse && this->ir_wrapper.is_ir_in<IRTYPE>(ir_to_fix, IRTypeNameList)) {
                this->instan_column_name(ir_to_fix, cur_stmt_root, is_replace_column,
                    ir_to_deep_drop);
            }
        }
        ...
        for (IR* ir_to_fix : ir_to_fix_vec) {
            ...
            if (ir_to_fix->get_ir_type() == IRTypeWindowName) {
    ...
    */
    for (const vector<IR*>& ir_to_fix_vec : all_substmt_ir_of_interest) {
        ...
        for (IR* ir_to_fix : ir_to_fix_vec) {
            if (ir_to_fix->get_data_type() == DataTableName && (cur_stmt_root->get_ir_type() == IRTypeCreateTableStmt || cur_stmt_root->get_ir_type() == IRTypeCreateTableAsStmt || cur_stmt_root->get_ir_type() == IRTypeCreateViewStmt || cur_stmt_root->get_ir_type() == IRTypeCreateIndexStmt) &&
...
#ifdef DEBUG
    cerr << "\nBEGIN cockroachdb_query_instantiator debugging. \n\n\n";
#endif
...
#ifdef DEBUG
    cerr << "\nEnd cockroachdb_query_instantiator debugging. \n\n\n";
#endif
