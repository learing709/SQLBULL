...
#include "mariadb_query_sequence.h"
...
{
    if (gen_idx < 4) {
        return p_rsg->generate_stmt_with_stmt_type(IRTypeCreateTable);
    } else if (gen_idx < 8) {
        return p_rsg->generate_stmt_with_stmt_type(IRTypeInsert);
    } else if (gen_idx < 14) {
        if (get_pct_hit(80)) {
            return p_rsg->generate_stmt_with_stmt_type(IRTypeAlter);
        } else {
            return p_rsg->generate_stmt_with_stmt_type(IRTypeCreateIndex);
        }
    } else if (gen_idx < 18) {
        return p_rsg->generate_stmt_with_stmt_type(IRTypeCreateIndex);
    } else if (gen_idx < 22) {
        // Any statement is welcomed.
        return p_rsg->generate_stmt_with_stmt_type(IRTypeVerbClause);
    } else {
        return p_rsg->generate_stmt_with_stmt_type(IRTypeSelect);
    }
}

QueryStmt* MariaDBQuerySequenceGenerator::generate_next_query_helper(RSG* p_rsg, FuzzingSequenceQueue* p_queue)
{
    if (this->gen_mode == GenUnknown) {
        // If already saved 100 query sequences, then mutate only.
        if (get_pct_hit(MariaDBFuzzerConfigurations::pct_mutate_ratio) || p_queue->get_saved_query_sequence_size() >= 100) {
    ...
    if (this->gen_mode == GenAllFromNew) {
        ...
        if (this->gen_idx >= this->p_saved_query_sequence->v_good_query_stmts.size()) {
            ...
            this->gen_idx++;
        } else if (get_pct_hit(MariaDBFuzzerConfigurations::pct_insert_new_in_mut) && this->p_saved_query_sequence->v_good_query_stmts.size() < 40) {
...
MariaDBQuerySequenceGenerator::MariaDBQuerySequenceGenerator()
{
    ...
    // append_stmt("DROP DATABASE IF EXISTS test123;");
    // append_stmt("CREATE DATABASE test123;");
    // append_stmt("USE test123;");
}
#undef append_stmt
