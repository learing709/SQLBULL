...
def is_token_str_or_literal(cur_token: str):
    if cur_token in {"IDENT", "SCONST", "BCONST", "BITCONST", "ICONST", "FCONST", "XCONST"}:
        return True
    else:
        return False
...
grammar_fd = open("assets/postgresql_gram.y", "r")
...
for cur_line in parser_rule_str.splitlines():
    if ":" in cur_line and "':'" not in cur_line and parent_level == 0:
        ...
        if is_mistake == False:
            ...
            if cur_keyword_has_rules == False and len(cur_keyword) != 0 and "error" not in cur_token_seq:
...
out_mapping_fd.write(f"ICONST,IRTypeINTEGER\n")
out_mapping_fd.write(f"BOOLEAN,IRTypeBOOLEAN\n")
out_mapping_fd.write(f"SCONST,IRTypeSTRING\n")
