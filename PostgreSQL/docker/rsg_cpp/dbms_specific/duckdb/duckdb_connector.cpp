...
unsigned char DuckDBConnector::run_target(char** argv, string cmd_str,
    int is_reset_server)
{
    ...
    if (WIFSIGNALED(status) && !stop_soon) {
        ...
        p_res_handl->set_tmp_cur_res(this->read_duckdb_output_and_reset_output_file());
...
void DuckDBConnector::init_forkserver(char** argv)
{
    ...
    if (!forksrv_pid) {
        ...
        execv("./duckdb", argv); // Used for start up sqlite3 ???
...
void DuckDBConnector::restart_dbms(char** argv)
{
    while (this->forksrv_pid == -1) {
        cerr << "Running init_forkserver() on DuckDB. " << endl;
        this->init_forkserver(argv);
    }

    this->run_target(argv, "SELECT 'abc';", true); // Run a dummy statement while resetting the DuckDB process.
}

void DuckDBConnector::write_to_testcase(string& in)
{
    /* After this memset, trace_bits[] are effectively volatile, so we
       must prevent any earlier operations from venturing into that
       territory. */
...
void DuckDBConnector::record_code_coverage(char** argv)
{
    /* EMPTY since trace_bits is synced with the DuckDB process. No need for explicit recording. */
}

string DuckDBConnector::read_duckdb_output_and_reset_output_file()
{
    // Alternative to read_file_to_str in util.h.
    // This is faster when using shared memory, faster in duckdb communication.
