...
#error "Error: Cannot recognize dbms_name: either cockroachdb nor duckdb. "
    ...
    for (int i = 0; i < 10; i++) {
        DATAAFFINITYTYPE data_affi = get_random_affinity_type();
        IR* new_func_expr = p_instantiator->constr_rand_func_with_affinity(data_affi);
        if (new_func_expr != nullptr) {
            cout << "Getting generated function: " << new_func_expr->to_string() << "\n";
        } else {
            cerr << "Error: Getting nullptr. \n";
        }
        new_func_expr->deep_drop();
    }

    cerr << "Next: test the IR generation with the function instantiation together. \n\n\n";

    for (int i = 0; i < 10; i++) {
        #if defined(mysqldb) || defined(mariadb)
        QueryStmt* gen_stmt = rsg->generate_complete_new_stmt_with_stmt_type(IRTypeSumExpr);
        #else
        QueryStmt* gen_stmt = rsg->generate_complete_new_stmt_with_stmt_type(IRTypeFuncExpr);
        #endif
