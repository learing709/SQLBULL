...
#include "duckdb_query_sequence.h"
...
QueryStmt* generate_stmt_with_idx(int gen_idx, RSG* p_rsg)
{
    if (gen_idx < 4) {
        if (get_pct_hit(50)) {
            return p_rsg->generate_stmt_with_stmt_type(IRTypeCreateStmt);
        } else {
            return p_rsg->generate_stmt_with_stmt_type(IRTypeCreateAsStmt);
        }
    } else if (gen_idx < 8) {
        return p_rsg->generate_stmt_with_stmt_type(IRTypeInsertStmt);
    } else if (gen_idx < 14) {
        return p_rsg->generate_stmt_with_stmt_type(IRTypeStmt);
    } else {
        return p_rsg->generate_stmt_with_stmt_type(IRTypeSelectStmt);
    }
}

QueryStmt* DuckDBQuerySequenceGenerator::generate_next_query_helper(RSG* p_rsg, FuzzingSequenceQueue* p_queue)
{
    if (this->gen_mode == GenUnknown) {
        if (get_pct_hit(DuckDBFuzzerConfigurations::pct_mutate_ratio)) {
    ...
    if (this->gen_mode == GenAllFromNew) {
        ...
        if (this->gen_idx >= this->p_saved_query_sequence->v_good_query_stmts.size()) {
            ...
            this->gen_idx++;
        } else if (get_pct_hit(DuckDBFuzzerConfigurations::pct_insert_new_in_mut)) {
...
DuckDBQuerySequenceGenerator::DuckDBQuerySequenceGenerator() {
    ...
    append_stmt("SET order_by_non_integer_literal=true;");
    append_stmt("SET scalar_subquery_error_on_multiple_rows=false;");
    append_stmt("SET allow_persistent_secrets=true;");
    append_stmt("SET allow_unredacted_secrets=true;");
    append_stmt("SET arrow_output_list_view=true;");
    append_stmt("SET binary_as_string=true;");
    append_stmt("SET default_null_order='NULLS_FIRST';");
    append_stmt("SET default_order='DESC';");
    append_stmt("SET enable_fsst_vectors=true;");
    append_stmt("SET enable_macro_dependencies=true;");
    append_stmt("SET enable_object_cache=true;");
    append_stmt("SET enable_view_dependencies=true;");
    append_stmt("SET immediate_transaction_mode=true;");
    append_stmt("SET lock_configuration=true;");
    append_stmt("SET autoinstall_known_extensions=1;");
    append_stmt("SET autoload_known_extensions=1;");
    append_stmt("SET lock_configuration=true;");
    // append_stmt("SET old_implicit_casting=true;");
    // append_stmt("SET preserve_insertion_order=true;");
    // append_stmt("SET produce_arrow_string_view=true;");
}
#undef append_stmt
