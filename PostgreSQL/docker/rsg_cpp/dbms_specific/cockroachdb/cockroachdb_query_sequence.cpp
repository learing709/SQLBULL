...
#include "cockroachdb_query_sequence.h"
...
QueryStmt* generate_stmt_with_idx(int gen_idx, RSG* p_rsg)
{
    if (gen_idx < 4) {
        if (get_pct_hit(50)) {
            ...
            return p_rsg->generate_stmt_with_stmt_type(IRTypeCreateTableAsStmt);
        }
    } else if (gen_idx < 8) {
        return p_rsg->generate_stmt_with_stmt_type(IRTypeInsertStmt);
    } else if (gen_idx < 14) {
        return p_rsg->generate_stmt_with_stmt_type(IRTypeStmtWithoutLegacyTransaction);
    } else {
        return p_rsg->generate_stmt_with_stmt_type(IRTypeSelectStmt);
    }
}

QueryStmt* CockroachDBQuerySequenceGenerator::generate_next_query_helper(RSG* p_rsg, FuzzingSequenceQueue* p_queue)
{
    if (this->gen_mode == GenUnknown) {
        if (get_pct_hit(CockroachDBFuzzerConfigurations::pct_mutate_ratio)) {
    ...
    if (this->gen_mode == GenAllFromNew) {
    ...
    else { // gen_mode == GenMut
        if (this->p_saved_query_sequence == nullptr) {
            //            this->p_saved_query_sequence = p_queue->get_random_query_sequence_from_queue();
        ...
        if (this->gen_idx >= this->p_saved_query_sequence->v_good_query_stmts.size()) {
            ...
            this->gen_idx++;
        } else if (get_pct_hit(CockroachDBFuzzerConfigurations::pct_insert_new_in_mut)) {
