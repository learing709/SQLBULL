...
#include "mariadb_query_instantiator.h"
...
void MariaDBQueryInstantiator::init_data_library()
{
...
void MariaDBQueryInstantiator::constr_sql_func_lib_helper(json& json_obj, vector<string>& v_all_func_str,
...
void MariaDBQueryInstantiator::constr_sql_func_lib(string func_types_str, vector<string>& v_all_func_str,
    map<DATAAFFINITYTYPE, vector<string>>& func_type_lib,
    map<string, vector<FunctionSignature>>& func_str_to_type_map)
{
...
void MariaDBQueryInstantiator::constr_key_pair_datatype_lib_helper(json& key_pair_json, vector<string>& v_all_key_str, map<string, DataAffinity>& mapped_key_pair)
{
...
void MariaDBQueryInstantiator::constr_key_pair_datatype_lib(string key_pair_str, vector<string>& v_all_key_str, map<string, DataAffinity>& mapped_key_pair)
{
...
bool MariaDBQueryInstantiator::fill_one_stmt_helper(IR* cur_stmt)
{
...
*/
vector<IR*> MariaDBQueryInstantiator::split_to_substmt(IR* cur_stmt,
...
bool MariaDBQueryInstantiator::connect_back(map<IR*, pair<int, IR*>>& m_save)
{
...
bool MariaDBQueryInstantiator::instan_preprocessing(IR* stmt_root,
...
bool MariaDBQueryInstantiator::remove_type_annotation(IR* cur_stmt_root,
...
void MariaDBQueryInstantiator::instan_database_schema_name(IR* ir_to_fix)
{
...
void MariaDBQueryInstantiator::instan_window_name(IR* ir_to_fix)
{
...
void MariaDBQueryInstantiator::instan_table_name(IR* ir_to_fix, bool& is_replace_table)
{
    ...
    if ((ir_to_fix->get_data_type() == DataTableName) && (ir_to_fix->get_data_flag() == ContextDefine || ir_to_fix->get_data_flag() == ContextReplaceDefine)) {
        ...
        if (this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeJoinTable)) {
            is_aliased = true;
...
void MariaDBQueryInstantiator::instan_table_alias_name(IR* ir_to_fix, IR* cur_stmt_root,
...
void MariaDBQueryInstantiator::instan_view_name(IR* ir_to_fix)
{
...
void MariaDBQueryInstantiator::instan_partition_name(IR* ir_to_fix)
{
...
void MariaDBQueryInstantiator::instan_index_name(IR* ir_to_fix, vector<IR*>& ir_to_drop)
{
...
DATAAFFINITYTYPE MariaDBQueryInstantiator::get_nearby_data_affinity(IR* ir_to_fix)
{
    ...
    vector<IRTYPE> v_capped_ir_types { IRTypeSelect, IRTypeSubquery };
...
string MariaDBQueryInstantiator::find_cloest_table_name(IR* ir_to_fix)
{
    string closest_table_name = "";
    IR* closest_table_ir = NULL;
    vector<DATATYPE> search_type { DataTableName, DataTableAliasName };
    vector<IRTYPE> cap_type { IRTypeSelect, IRTypeSubquery };
...
void MariaDBQueryInstantiator::instan_literal(IR* ir_to_fix, IR* cur_stmt_root,
    vector<IR*>& ir_to_deep_drop)
{
...
void MariaDBQueryInstantiator::instan_column_name(IR* ir_to_fix, IR*& cur_stmt_root, bool& is_replace_column, vector<IR*>& ir_to_drop)
{
    ...
    /* ContextUndefine scenario of the DataColumnName */
        ...
        if (cur_mapped_column_name_vec.size() > 0) {
            ...
            if (this->query_instan_data->v_table_alias_names_single.size() == 0 ||
                this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeLimitOptions) ||
                this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeUsingList)
            ) {
...
void MariaDBQueryInstantiator::instan_column_alias_name(IR* ir_to_fix, IR* cur_stmt_root,
    vector<IR*>& ir_to_deep_drop)
{
    ...
    if (ir_to_fix->get_data_type() == DataColumnAliasName) {
        ...
        if (this->ir_wrapper.is_ir_in(ir_to_fix, IRTypeSelectAlias)) {
...
void MariaDBQueryInstantiator::instan_sql_type_name(IR* ir_to_fix)
{
...
void MariaDBQueryInstantiator::instan_statistic_name(IR* ir_to_fix)
{
...
void MariaDBQueryInstantiator::instan_sequence_name(IR* ir_to_fix)
{
...
void MariaDBQueryInstantiator::instan_constraint_name(IR* ir_to_fix)
{
...
void MariaDBQueryInstantiator::instan_family_name(IR* ir_to_fix)
{
...
IR* MariaDBQueryInstantiator::constr_rand_func_with_affinity(DATAAFFINITYTYPE in_affi)
{
...
void MariaDBQueryInstantiator::instan_func_application(IR* ir_to_fix, IR* cur_stmt_root_ir, vector<IR*>& ir_to_deep_drop,
...
void MariaDBQueryInstantiator::map_create_view(IR* ir_to_fix, IR* cur_stmt_root,
    const vector<vector<IR*>> all_substmt_ir_of_interest)
{
    ...
    if (this->ir_wrapper.is_ir_in(cur_stmt_root, IRTypeViewSelect)) {
...
void MariaDBQueryInstantiator::map_create_view_column(IR* ir_to_fix,
    vector<IR*>& ir_to_deep_drop)
{

    IR* type_name_list = this->ir_wrapper.get_ancestor_node_matching_type(ir_to_fix, IRTypeViewList);
...
void MariaDBQueryInstantiator::instan_storage_param(IR* ir_to_fix, vector<IR*>& ir_to_deep_drop)
{
...
void MariaDBQueryInstantiator::instan_trigger_name(IR* ir_to_fix, vector<IR*>& ir_to_deep_drop)
{
...
bool MariaDBQueryInstantiator::instan_dependency(IR* cur_stmt_root,
    const vector<vector<IR*>> all_substmt_ir_of_interest)
{
...
void MariaDBQueryInstantiator::custom_setup_init_query_instan_data()
{
}
